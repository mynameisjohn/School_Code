...
int main() {
   ...
   T v=5.0, nu=100.0, A=50.0/1000.0, B=1.0, C=0.5, D=20.0/1000.0, Apost=25.0/1000, Cpost=0, tau, tauM=50.0, tauS=20.0;
   T xCum=0.0, V=0.0;
   T oldWeights[N];
   T dW[N];
   ...
   for (time=0;time<tMax;time++) {
      fprintf(output,"%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",
         ...
         xCum, V, post.x,
         pre[0].w, pre[1].w, dW[1]);
      xCum=0;   
  
        for (i=0;i<N;i++) {
         //dW[i]=nu*((1.0-pre[i].w)*pre[i].x*post.s-pre[i].w*post.x*pre[i].s);
         dW[i]=nu*(pre[i].x*post.s-post.x*pre[i].s);
         //dW[i]=dW;
      ...
      for (i=0;i<N;i++)
         pre[i].w=clamp((pre[i].w+dW[i]),(T)0,(T)1);
   }
