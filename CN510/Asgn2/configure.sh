...
echo "carter-gcc                     (same as carter but with GCC compiler for C/C++)"
...
echo "carter-impi-petsc34        (same as 'carter' but with PETSc 3.4)"
echo "carter-impi-petsc34-libs   (same as 'carter-impi-petsc34-libs' but with group libs)"
...
echo "rossmann-impi-petsc34 (same as above but with PETSc 3.4)"
echo "rossmann-impi-petsc34-libs      (same as above but without group libraries)"
...
echo "ubuntu-intel               (build on ubuntu with Intel compilers)"
echo "ubuntu-debug               (compiled with C++ STL container bounds checks)" 
...
echo "bluewaters-petsc34         (BlueWaters supercomputer with gcc and petsc34)"
echo "stampede                   (Stampede TACC system)"
...
if  [ $1 != "coates-intel64-mpich2" \
	...
	-a  $1 != "carter-debug-libs" \
        ...
        -a  $1 != "carter-impi-petsc34" \
        -a  $1 != "carter-impi-petsc34-libs" \
	...
	-a  $1 != "rossmann-impi-petsc34" \
	-a  $1 != "rossmann-impi-petsc34-libs" \
...
if [[ ( $1 == "sebuntu" ) || ( $1 == "ubuntu-debug" ) ]]; then
...
#	sed "s+#INSERT_LIBMESH_TECIO_ARCH+LIBMESH_TECIO_ARCH = $libmesh_architecture+" mkfiles/make.inc.sebuntu.temp > mkfiles/make.inc.sebuntu
   cp mkfiles/make.inc.sebuntu.template  mkfiles/make.inc.sebuntu
   if [ $1 == "ubuntu-debug" ]; then
      sed "s+#COMPILER_DEPENDENT_DEBUG_FLAG_INSERT+COMPILER_DEPENDENT_DEBUG_FLAG = -D_GLIBCXX_DEBUG+" -i mkfiles/make.inc.sebuntu
   fi
  # rm mkfiles/make.inc.sebuntu.temp
elif [ $1 == "ubuntu-intel" ]; then
     cp mkfiles/make.inc.ubuntu_intel.template  mkfiles/make.inc.ubuntu-intel  
...
elif [ $1 == "rossmann-impi-petsc34-libs" ]; then
	libs_top=/apps/group/ncn/rossmann/petsc34/libs
	sed "s+#INSERT_LIB_TOP+$libs_top+g" mkfiles/make.inc.rossmann-impi-petsc34-libs.template > mkfiles/make.inc.rossmann-impi-petsc34-libs	
...
elif [ $1 == "carter-impi-petsc34-libs" ]; then
        libs_top=/apps/group/ncn/carter/PETSc34/libs
        sed "s+#INSERT_LIB_TOP+$libs_top+g" mkfiles/make.inc.carter-impi-petsc34-libs.template > mkfiles/make.inc.carter-impi-petsc34-libs
fi
