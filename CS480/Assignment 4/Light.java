...
public class Light  
{
	...
	private boolean shadow_on; // Turns on or off shadows
	...
	public float[] addLight(Point pI, Ray lRay, RenderObjectCollection objects, float[] N, float[] L)
	{
		float t=0;
		float[] rgb=new float[4];

		t=objects.findIntersections(lRay);

		if (t==4000||t<=EPSILON)
		{//If nothing is hit on the way to the light
			//System.out.println("hinga");
			lRay.n++;
			return this.rgb;
		}
		else
			//Otherwise return the rgb array with only 0's, since no contribution from that light
		{
	...
	{
		return (this.type==LightType.INFINITE);
	}
	...
	public void setLight( GL gl )
	{
		...
		if( type == LightType.POINT || type == LightType.INFINITE )
		{
			...
			gl.glLightfv( glLightId, GL.GL_AMBIENT, zeroLight, 0);
			gl.glLightfv( glLightId, GL.GL_DIFFUSE, rgb, 0);
			gl.glLightfv( glLightId, GL.GL_SPECULAR, rgb, 0);
			gl.glLightfv( glLightId, GL.GL_POSITION, pos, 0);
			gl.glLightfv( glLightId, GL.GL_SPOT_DIRECTION, dir, 0);
			gl.glLightf ( glLightId, GL.GL_SPOT_EXPONENT, angularAttenuation);
			gl.glLightf ( glLightId, GL.GL_SPOT_CUTOFF, spotCutoff);
		...
		else 
		{
			...
			// specular values. 
			gl.glLightfv( glLightId, GL.GL_AMBIENT, rgb, 0);
			gl.glLightfv( glLightId, GL.GL_DIFFUSE, zeroLight, 0);
			gl.glLightfv( glLightId, GL.GL_SPECULAR, zeroLight, 0);
