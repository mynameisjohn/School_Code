...
public class Polygon {
	...
	/**Boolean that activates if the polygon is self intersecting*/
	//public boolean selfIntersecting = false;

	//public final float[] extremes = extremes();  I had tried to make this a class constant, but I kept having bugs in my programs. I'll leave this till a later assignment. 
	...
	public boolean isConcave() {
		...
		for (int i=0;i<this.vertices.size();i++)
		{
			...
			if (z<0) {neg=true; CCW++;}
			if (CW>CCW)
			{//This method  for CW/CCW seems a bit barbaric. I'm curious about what happens when the two integers are equal. 
				this.clockWise = true;
		...
		}//If this portion of the code is reached the polygon is not concave. 
		return false;
	}
	...
	public boolean isOverlapping(Polygon p){
		...
		ArrayList<float[]> edgeVectors = p.computeEdgeVectors();
		
		for (int i=0;i<p.vertices.size();i++)
		{
			float x0 = p.vertices.get(i).x;
			float y0 = p.vertices.get(i).y;
			float u;
			for (u=0;u<=1;u+=(float)0.05)
			{
				if (this.isInside(x0+u*edgeVectors.get(i)[0], y0+u*edgeVectors.get(i)[1]))
				{return true;}
			}
...
/*
		...
		for (int i=0;i<this.vertices.size();i++)
		{
			...
			{//We use a simple nested for loop to check each vector from this polygon with each vector from the other polygon. It is a lot of calculation, but I couldn't think of a better way. 
		...
		else
		{
			...
			for (int i=0;i<edgeVectors.size();i++)
			{
				//if (edgeVectors.get(i)[1]!=0)
				{
					...
					if (point[0]!=-1&&point[0]<x&&point[1]!=this.vertices.get(i).y)//-1 Should only be returned if there is no intersection (I think this is where the problem is)
						n++;//If we have a good intersection, the number of intersection is incremented. 
				}
			}
	...
	public boolean isSelfintersecting()
	{
		for (int i=0;i<this.vertices.size();i++)
		{
			...
			{
				float[] poi = intersection(this.vertices.get(i).x, this.vertices.)
			}
		}
	...
	/**A method I wrote that solves two equations for two unknowns to find the intersection points of two vectors. 
	 * The parameters are the initial points of the two vectors as well as the actual vector, which is added in the following way:
	 * [x0, y0]+u*v0 and [x1, y1]+v*v1*/
	public static float[] intersection (float x0, float y0, float x1, float y1, float[] v0, float[] v1)
	{
		...
		if (u<0||u>1||v0[1]==0){//If the intersection point is out of the range of the edge vector. 
	...
	/**A method I wrote to find the "boundary box" coordinates for a polygon. 
	 * That is, the extreme x and y values that form a quadrilateral around the polygon in the form
	 * {xmin, ymin, xmax, ymax}
	 * @return
	 */
