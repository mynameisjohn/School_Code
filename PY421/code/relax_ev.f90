...
SUBROUTINE relax(xw,yw,pxa)
  ...
  REAL(REAL8) :: vmax, lambda
  LOGICAL, SAVE :: initialized=.FALSE.
  REAL(REAL8), SAVE :: omega
  ...
  IF(img==1) THEN
     ...
     WRITE(*,'("Enter the number of images, the number of relaxation steps&
          & per image")')
     WRITE(*,'("and the value of omega: ")',ADVANCE='NO')     
     READ *,nimg,nstp,omega
     PRINT *
  ...
  IF(img>0.AND.img<=nimg) THEN
     ...
     ! Insert here the instructions that apply nstp times the
     ! operation v->v'=v-omega*A*v (see the text of the assignment.) 
     ! After each step increase the variable step by 1.

     ! Insert here the instructions that normalize the potential

     ! Insert here the estimate of the eigenvalue and print out
     ! the number of steps done so far and the estimate

  ENDIF

  ! Before displaying we multiply v by -20 to enhance the color 

  vn=-20*v
  CALL v2pxa(pxa,vn,inside,size)  
  ...
  IF(img==nimg) THEN
     ...
     PRINT *
     WRITE (*,'("Enter any number to stop the program:  ")',ADVANCE='NO')
     READ *,i
     STOP
